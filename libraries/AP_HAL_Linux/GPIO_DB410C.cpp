...
GPIO_DB410C::GPIO_DB410C()
{}

void GPIO_DB410C::init()
{
...
void GPIO_DB410C::pinMode(uint8_t pin, uint8_t output)
{
    uint8_t bank = pin/32;
...
int8_t GPIO_DB410C::analogPinToDigitalPin(uint8_t pin)
{
    return -1;
}


uint8_t GPIO_DB410C::read(uint8_t pin) {
...
void GPIO_DB410C::write(uint8_t pin, uint8_t value)
{
    uint8_t bank = pin/32;
    uint8_t bankpin = pin & 0x1F;
    if (bank >= LINUX_GPIO_NUM_BANKS) {
...
void GPIO_DB410C::toggle(uint8_t pin)
{
    write(pin, !read(pin));
}

/* Alternative interface: */
AP_HAL::DigitalSource* GPIO_DB410C::channel(uint16_t n) {
    return new DigitalSource(n);
}

/* Interrupt interface: */
bool GPIO_DB410C::attach_interrupt(uint8_t interrupt_num, AP_HAL::Proc p, uint8_t mode)
{
    return true;
}

bool GPIO_DB410C::usb_connected(void)
{
    return false;
}
