...
void RCOutput_DB410C::init()
{
    _dev = hal.i2c_mgr->get_device(PCA9685_I2C_BUS, PCA9685_I2C_ADDR);

    hal.scheduler->register_timer_process(FUNCTOR_BIND_MEMBER(&RCOutput_DB410C::_update, void));
}

void RCOutput_DB410C::set_freq(uint32_t chmask, uint16_t freq_hz)
{
...
uint16_t RCOutput_DB410C::get_freq(uint8_t ch)
{
    return _frequency;
}

void RCOutput_DB410C::enable_ch(uint8_t ch)
{

}

void RCOutput_DB410C::disable_ch(uint8_t ch)
{
    //write(ch, 0);

}

void RCOutput_DB410C::write(uint8_t ch, uint16_t period_us)
{
...
uint16_t RCOutput_DB410C::read(uint8_t ch)
{
//    if(ch >= PWM_CHAN_COUNT){
//        return 0;
//    }
...
void RCOutput_DB410C::read(uint16_t* period_us, uint8_t len)
{
//    for (int i = 0; i < len; i++)
//        period_us[i] = read(0 + i);
}

void RCOutput_DB410C::_update(void)
{
//    int i;
//
...
void RCOutput_DB410C::cork(void)
{
//    _corked = true;
}

void RCOutput_DB410C::push(void)
{
//    _corked = false;
}

#endif // CONFIG_HAL_BOARD_SUBTYPE == HAL_BOARD_SUBTYPE_LINUX_DB410C
